    ...
    <div class="aC-input">
      ...
      >      
        <el-form-item label="用户名" prop="username">
          <el-input v-model="ruleForm.username" @change="checkUsername" placeholder="请输入用户名"></el-input>
        </el-form-item>
        <el-form-item label="初始密码" prop="password">
          <!-- <el-input v-model="ruleForm.password" v-show="false"></el-input> -->
          <el-input
            @focus="type='password'"
        ...
        <el-form-item label="公司法人" prop="companyLerep">
        ...
        <el-form-item label="行业类别" prop="companyCategory">
          <el-select v-model="ruleForm.companyCategory" placeholder="请选择行业类别">
            <el-option
              v-for="(item,index) in companyCate"
              :label="item.label"
              :key="index"
              :value="item.value"
            ></el-option>
        ...
        <div class="aC-image">
					<!-- 192.168.1.198/sioo-client-web/oss/file/upload  /fastdfs/upload/image/sample-->

          <el-form-item label="上传资质" prop="qualiUrl">
            <el-upload
              class="avatar-uploader"
              action="http://gateway.paasw.com/sioo-admin-web/oss/file/upload"
...
export default {
  data() {
    return {
      ...
      // myHeaders: { 'Content-Type': 'multipart/form-data' },
			uploadHeader:{"X-Auth-Token" : sessionStorage.getItem('myToken_admin')},
      ...
      rules: {
        username: [
          ...
          { pattern: /^[0-9a-zA-Z]+$/, message: "用户名不能为中文，只能为数字和字母" },
        ...
        companyLerepCard: [
          { required: true, pattern:/^(([1][1-5])|([2][1-3])|([3][1-7])|([4][1-6])|([5][0-4])|([6][1-5])|([7][1])|([8][1-2]))\d{4}(([1][9]\d{2})|([2]\d{3}))(([0][1-9])|([1][0-2]))(([0][1-9])|([1-2][0-9])|([3][0-1]))\d{3}[0-9xX]$/,message: "请输入正确的身份证号", trigger: "blur" }
        ],
        companyAddress: [
        ...
        creditCode: [
          ...
          { message: "长度在 0 到 20 个字符", trigger: "blur" }
        ],
        companyCategory: [
        ...
        companyContactsCellphone: [
          { required:true, message: "请输入正确的联系人电话", trigger: "blur" },
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      console.log(res)
      console.log(file)
			// localStorage.setItem("filePath",res.filePath)
